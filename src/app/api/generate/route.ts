...
export async function POST(request: NextRequest) {
  try {
    ...
    // (ComfyUI v0.30 on 127.0.0.1:8190 — the only box with MiniMax nodes);
    // resolveWorkerForLane already put the sidecar first for this kind, so
    // comfyBase above targets it. Frames snap to H3's 17k+5 grid; optional
    ...
    if (modelProfile.kind === "minimax-h3") {
      ...
      const h3Height = body.height || modelProfile.defaultHeight || 768;
      const length = body.duration
        ? validateH3FrameGrid(Math.round(body.duration * h3Fps))
        : (modelProfile.defaultLength ?? 124);
      const seed = req.seed ?? Math.floor(Math.random() * 2147483647);

      const template = loadTemplate(modelProfile.templateFile ?? "minimax_h3.json");
      const workflow = buildMiniMaxH3({
        template,
        prompt: req.prompt,
        imageName,
        endImageName,
      ...
      const item: VideoGenerationItem = {
        ...
        prompt: req.prompt,
        comfyPromptId: comfyResponse.prompt_id,
        width: workflow.extra_data.width ?? h3Width,
        height: workflow.extra_data.height ?? h3Height,
        fps: h3Fps,
        frames: workflow.extra_data.length,
