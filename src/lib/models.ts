...
export type VideoModelId =
  ...
  // proven 2026-08-04: I2V ~4x faster than Wan; FLF2V keyframing; PromptRelay
  // in-generation beat scheduling. All explicit selection, never defaults)
  | "ltx23-i2v-fast"
  | "ltx23-flf2v"
  | "ltx23-beats"
  | "ltx23-camera"
...
export const VIDEO_MODEL_PROFILES: VideoModelProfile[] = [
  ...
  },
  {
    ...
    // per beat, 81-frame clips chained via anchor+prev latents, scene lock
    // held indefinitely. HERO = 10 steps cfg 4.0 with NO lightning distill
    // (the distill tames motion and weakens beat adherence — pilot-verified);
    // budget ~25-30 min/beat. Graph is code-generated by buildSviChain (no
    // template file — beat count is variable). Explicit selection only.
    id: "svi-chain",
    name: "SVI 2.0 Pro Chain (Hero)",
    shortName: "SVI Hero",
    description:
      "Drift-free long-form video: one prompt per story beat, 81-frame clips chained without scene melt (SVI 2.0 Pro rank-128 pair on Wan 2.2 I2V). Hero grade: 10 steps cfg 4.0, no distill — full motion + beat adherence, ~25-30 min per beat. 1280x720 @ 16fps. Needs the v0.30 sidecar. Anchor image required.",
    kind: "svi-chain",
    backend: "comfyui",
    checkpoint: "wan2.2_i2v_high+low_noise_14B_fp8_scaled + SVI_v2_PRO rank128",
    textEncoder: "umt5_xxl_fp8_e4m3fn_scaled.safetensors",
    steps: 10,
    videoCfg: 4,
    audioCfg: 0,
    ...
    defaultWidth: 1280,
    defaultHeight: 720,
    defaultLength: 81, // per beat; total = 81 x beats
    requiresImage: true,
    supportsEndImage: false,
  },
  {
    // SVI 2.0 Pro DRAFT chain — same graph at the pilot's distill settings
    // (6 steps 3/3, cfg 1.5, lightning 0.6 high / 1.0 low): ~16 min/beat,
    // tamer motion + looser beat adherence (documented SVI x LightX2V
    // conflict). Iterate beats/anchor here, then re-run the hero.
    id: "svi-chain-draft",
    name: "SVI 2.0 Pro Chain (Draft)",
    shortName: "SVI Draft",
    description:
      "Fast SVI chain draft: same drift-free beat chaining at 6-step distill (~16 min per beat), with reduced motion energy and looser beat adherence. Iterate the beat list cheaply, then promote to svi-chain (hero) for the real render.",
    kind: "svi-chain",
    backend: "comfyui",
    checkpoint: "wan2.2_i2v 14B fp8 + SVI_v2_PRO + wan22-lightning pair",
    textEncoder: "umt5_xxl_fp8_e4m3fn_scaled.safetensors",
    steps: 6,
    videoCfg: 1.5,
    audioCfg: 0,
  ...
  {
    // Sidecar LTX 2.3 I2V — the fast cinematic motion lane proven 2026-07-28
    // (identity-preserving I2V from a strong still in ~76s at 1216x512 —
    // ~4x faster than Wan 2.2 I2V) running CORE LTXV nodes on the v0.30
    // sidecar worker with the CORRECT bf16 Gemma encoder (the e4m3fn quant
    // silently drops its tensors on the 0.30 core → unconditioned output —
    // see the 2026-08-04 seed-sweep post-mortem). 8-step distilled, cfg 1,
    // 24fps native (no RIFE). When the request also carries an END image the
    // generate route swaps in the FLF2V keyframe template (ltx23-flf2v).
    id: "ltx23-i2v-fast",
    name: "LTX 2.3 Sidecar I2V Fast",
    shortName: "LTX I2V",
    description:
      "The fast cinematic motion lane: animate a strong still with a motion-only prompt (describe ONLY what changes after frame 1) — LTX 2.3 22B distilled fp8, 8 steps, 1216x512 @ 24fps, ~1.5 min/clip. ~4x faster than Wan I2V with identity preserved. Add an end image for FLF2V keyframing.",
    kind: "ltx-sidecar",
    backend: "comfyui",
    templateFile: "ltx23_i2v_sidecar.json",
    checkpoint: "ltx-2.3-22b-distilled-1.1_transformer_only_fp8_scaled.safetensors",
    textEncoder: "comfy_gemma_3_12B_it.safetensors",
    steps: 8,
    videoCfg: 1,
    audioCfg: 0,
    includeAudio: false,
    ...
    defaultWidth: 1216,
    defaultHeight: 512,
    defaultLength: 65, // 8n+1 grid, ~2.7s @ 24fps
    requiresImage: true,
    supportsEndImage: true,
  },
  {
    // Sidecar LTX 2.3 FLF2V — first+last keyframe travel (proven 2026-08-04:
    // opens on frame A, settles on frame B, coherent invented in-betweens).
    // Same 8-step recipe + distilled LoRA 0.5, two LTXVAddGuide anchors.
    // Usually reached automatically from ltx23-i2v-fast + an end image;
    // selectable directly for explicit keyframe work.
    id: "ltx23-flf2v",
    name: "LTX 2.3 Sidecar FLF2V Keyframes",
    shortName: "LTX FLF2V",
    description:
      "First+last frame keyframing: give two stills and a travel prompt — the camera moves from A to B inventing coherent in-betweens. LTX 2.3 distilled 8-step, 1216x512 @ 24fps. Requires BOTH imageUrl and endImageUrl.",
    kind: "ltx-sidecar",
    backend: "comfyui",
    templateFile: "ltx23_flf2v_sidecar.json",
    checkpoint: "ltx-2.3-22b-distilled-1.1_transformer_only_fp8_scaled.safetensors",
    textEncoder: "comfy_gemma_3_12B_it.safetensors",
    steps: 8,
    videoCfg: 1,
  ...
  {
    // Sidecar LTX 2.3 BEATS — PromptRelay in-generation prompt scheduling
    // (gated test PASSED 2026-08-04 on this exact distilled fp8 8-step
    // recipe: beats switch on time, no color shift). The PROMPT is the beat
    // list: pipe-separated segments ("he reads | he stands | he walks to the
    // window"), optional [start-end] weights; optional `globalPrompt` body
    // param anchors style/character across all beats (empty = the node uses
    // the first segment). Text-only (t2v) in v1. Needs the sidecar's
    // ComfyUI-PromptRelay custom node pack.
    id: "ltx23-beats",
    name: "LTX 2.3 Sidecar Multi-Beat",
    shortName: "LTX Beats",
    description:
      "Multi-beat storytelling inside ONE generation: prompt = pipe-separated beats (\"A | then B | then C\", optional [0-50] weights), optional globalPrompt for the persistent style/character anchor. LTX 2.3 distilled 8-step via PromptRelay, 1216x512 @ 24fps, ~2 min for 4s. Text-only.",
    kind: "ltx-sidecar",
    backend: "comfyui",
    templateFile: "ltx23_beats_sidecar.json",
    checkpoint: "ltx-2.3-22b-distilled-1.1_transformer_only_fp8_scaled.safetensors",
    textEncoder: "comfy_gemma_3_12B_it.safetensors",
    steps: 8,
    videoCfg: 1,
    ...
    defaultWidth: 1216,
    defaultHeight: 512,
    defaultLength: 97, // 8n+1 grid, ~4s @ 24fps — room for 3 beats
    requiresImage: false,
    supportsEndImage: false,
  },
  {
    // Sidecar LTX 2.3 CAMERA — Cameraman IC-LoRA v2 motion transfer (gated
    // test PASSED 2026-08-06: a synthetic pan-right reference reproduced in a
    // generated scene). Runs the 22B DEV fp8 transformer + distilled speed
    // LoRA 0.6 + the IC-LoRA @ 1.0 with the AV latent path (audio out). The
    // cameraMove body param picks a reference clip from the on-box camlib/
    // library; per the LoRA author, do NOT put camera verbs in the prompt.
    // NOTE: dev fp8 is a DIFFERENT model family from the distilled lanes —
    // switching families on the sidecar thrashes RAM (~8 min extra); batch
    // camera jobs together.
    id: "ltx23-camera",
    name: "LTX 2.3 Sidecar Camera Transfer",
    shortName: "LTX Camera",
    description:
      "Real camera moves by reference: pick a cameraMove (push_in, pull_back, pan_left, pan_right, tilt_up, tilt_down) and the Cameraman IC-LoRA transfers that motion onto your still + scene prompt. Keep camera verbs OUT of the prompt. 960x544 @ 24fps with audio, dev-model quality.",
    kind: "ltx-sidecar",
    backend: "comfyui",
    templateFile: "ltx23_camera_sidecar.json",
    checkpoint: "ltx-2.3-22b-dev_transformer_only_fp8_scaled.safetensors",
    textEncoder: "comfy_gemma_3_12B_it.safetensors",
    steps: 8,
    videoCfg: 1,
    audioCfg: 0,
    ...
    defaultWidth: 960,
    defaultHeight: 544,
    defaultLength: 97, // 8n+1 grid, ~4s @ 24fps
    requiresImage: true,
    supportsEndImage: false,
  },
  {
